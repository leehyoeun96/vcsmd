...
void mallocArray()
{
	control_cnt = (int *)malloc(sizeof(int) * get_cfParam("ECAT.num_motors"));
	for (int i = 0; i < get_cfParam("ECAT.num_motors"); i++)
		control_cnt[i] = 3;
	motor_info = (Motor_info *)malloc(sizeof(Motor_info) * get_cfParam("ECAT.num_motors"));
	master_info.sc_epos = (ec_slave_config_t **)malloc(sizeof(ec_slave_config_t *) * get_cfParam("ECAT.num_motors"));
	master_info.slave_state = (ec_slave_config_state_t *)malloc(sizeof(ec_slave_config_state_t) * get_cfParam("ECAT.num_motors"));
	master_info.domain = (ec_domain_t **)malloc(sizeof(ec_domain_t *) * get_cfParam("ECAT.num_motors"));
	master_info.domain_state = (ec_domain_state_t *)malloc(sizeof(ec_domain_state_t) * get_cfParam("ECAT.num_motors"));
	master_info.domain_pd = (uint8_t **)malloc(sizeof(uint8_t *) * get_cfParam("ECAT.num_motors"));
}
void initSlaveInfo() //using in EcatMgr.cpp
{
	int idx;

	for (idx = 0; idx < get_cfParam("ECAT.num_motors"); ++idx)
	{
...
int checkDomainState()
{
	int i = 0;
	int ret_val = 0;
	for (i = 0; i < get_cfParam("ECAT.num_motors"); i++)
	{
...
void profileMotorSpeed(int slave, int accel, int decel, int vel)
{
...
int ecatUp()
{
	...
	for (i = 0; i < get_cfParam("ECAT.num_motors"); i++)
	{
		if (i == get_cfParam("ECAT.motor_id_steer"))
		{
			profileMotorSpeed(i, 5000, 5000, 5000);
		}
		else
			profileMotorSpeed(i, 150000, 150000, 150000);
		//if(i==get_cfgcfParam("EC.brake_id"))
		//profileMotorSpeed(i, 1000, 1000, 1000);
	}
