...
int CCdecide()
{
    ...
    if (cc_print_freq == 0)
    {
        ...
        printf("##     plus_margin = %d       \n", plus_margin);
        printf("##     minus_margin = %d      \n", minus_margin);
        printf("##     target Vel = %lf        \n", new_tvel);
        printf("##      current Vel = %lf      \n", actual_vel);
        printf("##    lookahead_tvel = %lf      \n", lookahead_tvel);
        printf("##       power = %lf      \n", power);
...
int CCdecel()
{
    ...
    homingThrottle();
    /*
    if (pos > (double)get_cfParam("Brake.virtual_max_position"))
    {
        brakeModule("Brake.target_position", get_cfParam("Brake.virtual_max_position"));
    }
    else 
    */
    if (pos < (double)get_cfParam("Brake.virtual_min_position"))
    {
        homingBrake();
    }
    else
...
void initHA_avc()
{
    ...
    avc_HA->setCondition(AVC_SELFDRIVING, NULL, AVC_FINISH);
    avc_HA->setCondition(AVC_ESTOP, NULL, AVC_FINISH);
}
int operateHA_avc()
{
