...
int decidePullover()
{
    ...
    /* if(get_var("Current_Velocity", "value")==0)
  {
    ...
    {
        init_vel = get_rtParam_double("OBD.actual_velocity", "value") / 3.6;
    }

    cur_vel = get_rtParam_double("OBD.actual_velocity", "value") / 3.6;
...
class ST_S_max2S_max : public Condition
{
  public:
    bool check(HybridAutomata *HA)
    {
        if (get_slave_virtual_current_pos(get_cfParam("ECAT.motor_id_steer")) != get_cfParam("Steerwheel.virtual_max_position"))
            return true;
        else
            return false;
    }
...
class ST_S_min2S_min : public Condition
{
  public:
    bool check(HybridAutomata *HA)
    {
        if (get_slave_virtual_current_pos(get_cfParam("ECAT.motor_id_steer")) != get_cfParam("Steerwheel.virtual_min_position"))
            return true;
        else
            return false;
    }
