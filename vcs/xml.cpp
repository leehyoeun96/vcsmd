...
{
    double physical_range_pos = abs(get_cfParam("Steer.max_position") - get_cfParam("Steer.min_position"));
    double virtual_range_pos = abs(get_cfParam("Steer.virtual_max_position") - get_cfParam("Steer.virtual_min_position"));
    cfParam.insert(pair<string, int>("Steer.convert_position", physical_range_pos / virtual_range_pos));
}
static void
...
getNode_SteerWheel_children(xmlNode *node)
{
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next)
    {
        if (cur_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("Steer.max_position", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"origin")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("Steer.start_position", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"rightmost")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("Steer.min_position", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"virtualleftmost")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("Steer.virtual_min_position", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"virtualrightmost")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("Steer.virtual_max_position", atoi(s)));
                xmlFree(s);
            }
        }
    }
...
static void
getNode_accelPID_children(xmlNode *node)
{
    xmlNode *cur_node = NULL;
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next)
    {
        if (cur_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("CruiseControl.accel_gain_p", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"ki")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("CruiseControl.accel_gain_i", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"kd")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("CruiseControl.accel_gain_d", atoi(s)));
                xmlFree(s);
            }
        }
    }
...
static void
getNode_decelPID_children(xmlNode *node)
{
    xmlNode *cur_node = NULL;
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next)
    {
        if (cur_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("CruiseControl.decel_gain_p", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"ki")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("CruiseControl.decel_gain_i", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"kd")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("CruiseControl.decel_gain_d", atoi(s)));
                xmlFree(s);
            }
        }
    }
...
getNode_CruiseControl_children(xmlNode *node)
{
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next)
    {
        if (cur_node->type == XML_ELEMENT_NODE)
        {
            if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"accelpid")))
            {
                getNode_accelPID_children(cur_node->children);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"decelpid")))
            {
                getNode_decelPID_children(cur_node->children);
            }
...
getNode_MAIN_children(xmlNode *node)
{
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next)
    {
        if (cur_node->type == XML_ELEMENT_NODE)
        {
            if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"connectclient")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
...
getNode_EStopReceiver_children(xmlNode *node)
{
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next)
    {
        if (cur_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("HVI.estop_remote_run", atoi(s)));
                xmlFree(s);
            }
            else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"pausegpio")))
            {
                xmlChar *s = xmlNodeGetContent(cur_node);
                cfParam.insert(pair<string, int>("HVI.estop_remote_pause", atoi(s)));
                xmlFree(s);
            }
        }
    }
